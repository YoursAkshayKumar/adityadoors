...
import { useGetAllContactsQuery } from "@/redux/contact/contactApi";
...
const ContactTable = ({ cls, searchTerm }: { cls?: string; searchTerm?: string }) => {
  const { data: contacts, isError, isLoading } = useGetAllContactsQuery();
  const [selectedContactId, setSelectedContactId] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const allItems = (contacts?.result || []) as any[];
  const filtered = searchTerm
    ? allItems.filter((it) => {
        ...
        return name.includes(searchLower) || email.includes(searchLower) || phone.includes(searchLower) || subject.includes(searchLower);
      })
    : allItems;
                  ...
                  {currentItems.map((item) => {
                    ...
                    return (
                      <tr key={item._id} className="bg-white border-b border-gray6 last:border-0">
                        ...
                        <td className="px-3 py-3 font-normal text-[#55585B] text-end">
                          <div className="text-sm max-w-[200px] truncate" title={item.subject || "N/A"}>
                            {item.subject || "N/A"}
                          </div>
                        </td>
                        <td className="px-3 py-3 font-normal text-[#55585B] text-end">
                        ...
                        <td className="px-9 py-3 text-end">
                          <div className="flex items-center justify-end space-x-2">
                            <ContactEditDelete id={item._id} onViewClick={handleViewClick} />
                          </div>
                        </td>
                      </tr>
                    );
