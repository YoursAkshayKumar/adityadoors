...
import InquiryViewModal from "./inquiry-view-modal";
...
const InquiryTable = ({ cls, searchTerm }: { cls?: string; searchTerm?: string }) => {
  ...
  const filtered = searchTerm
    ? allItems.filter((it) => {
        ...
        const productName = (it.productName || it.product?.name || "").toString().toLowerCase();
        const searchLower = searchTerm.toLowerCase();
        return name.includes(searchLower) || email.includes(searchLower) || productName.includes(searchLower);
      })
    : allItems;
                      ...
                      <tr key={item._id} className="bg-white border-b border-gray6 last:border-0">
                        ...
                        <td className="px-3 py-3 font-normal text-[#55585B] text-end">
                          <div className="text-sm">
                            <div className="font-medium">{item.productName || item.product?.name || "N/A"}</div>
                            <div className="text-xs text-gray-400">{item.productCategory || item.product?.category || "N/A"}</div>
                          </div>
                        </td>
                        <td className="px-3 py-3 font-normal text-[#55585B] text-end">
                        ...
                        <td className="px-9 py-3 text-end">
                          <div className="flex items-center justify-end space-x-2">
                            <InquiryEditDelete id={item._id} onViewClick={handleViewClick} />
                          </div>
                        </td>
                      </tr>
                    );
